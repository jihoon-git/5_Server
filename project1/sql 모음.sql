...
COMMENT ON COLUMN "MEMEBER"."MEMBER_NO" IS '회원번호(SEQ_MEMBER_NO)';

COMMENT ON COLUMN "MEMEBER"."MEMBER_EMAIL" IS '회원 이메일(아이디)';

COMMENT ON COLUMN "MEMEBER"."MEMBER_PW" IS '회원 비밀번호(암호화)';

COMMENT ON COLUMN "MEMEBER"."MEMBER_NICKNAME" IS '회원 닉네임(중복X)';

COMMENT ON COLUMN "MEMEBER"."MEMBER_TEL" IS '휴대폰 번호(-없음)';

COMMENT ON COLUMN "MEMEBER"."MEMBER_ADDRESS" IS '회원 주소';

COMMENT ON COLUMN "MEMEBER"."PROFILE_IMG" IS '프로필 이미지 경로';

COMMENT ON COLUMN "MEMEBER"."ENROLL_DATE" IS '회원 가입일';

COMMENT ON COLUMN "MEMEBER"."MEMBER_DEL_FL" IS '탈퇴여부(N:탈퇴X , Y:탈퇴O)';

COMMENT ON COLUMN "MEMEBER"."AUTHORITY" IS '회원 권한(1:일반, 2:관리자)';
...
COMMENT ON COLUMN "BOARD_IMG"."IME_ORDER" IS '이미지 파일 순서 번호';
...
COMMENT ON COLUMN "BOARD_TYPE"."BOARD_CODE" IS '게시판 코드 번호(SEQ_BOARD_CODE)';
...
ALTER TABLE "BOARD" ADD CONSTRAINT "FK_MEMEBER_TO_BOARD_1" FOREIGN KEY (
	"MEMBER_NO"
)
REFERENCES "MEMEBER" (
	"MEMBER_NO"
...
ALTER TABLE "BOARD_LIKE" ADD CONSTRAINT "FK_MEMEBER_TO_BOARD_LIKE_1" FOREIGN KEY (
	"MEMBER_NO"
)
REFERENCES "MEMEBER" (
	"MEMBER_NO"
...
ALTER TABLE "COMMENT" ADD CONSTRAINT "FK_MEMEBER_TO_COMMENT_1" FOREIGN KEY (
	"MEMBER_NO"
)
REFERENCES "MEMEBER" (
	"MEMBER_NO"
